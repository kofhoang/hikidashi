...
- A [Scaleway](https://console.scaleway.com) account + the [`scw` CLI](https://github.com/scaleway/scaleway-cli) (`scw init`) and `jq` if you use `deploy.sh`
...
2. **Enable AuthKit** (the hosted auth UI).
...
5. **Add Google sign-in.** Under the authentication/social-connection settings, enable **Google
   OAuth**. This is how you'll log in.
...
2. Create the credentials it authenticates with — an **IAM application + API key**:
   ...
   - Give it a policy allowing access to Serverless SQL in your project.
...
> **Shortcut:** with `scw` configured and `jq` installed, **`./deploy.sh`** does steps 1–5
> automatically (build → push → create/update container → print the MCP URL). Set the env vars listed
> at the top of the script. On the very first run set `AUTH_AUDIENCE` to any placeholder, then do
> step 6 and re-run `./deploy.sh` with the real URL. The manual steps below are the same thing by hand.
...
- **Resources:** the smallest tier that boots .NET (≈512 MB) is plenty.
...
- **Environment variable** — `Auth__Audience` = leave as a placeholder for now; set it in step 6.
...
If hikidashi's `Audience`, the WorkOS Resource Indicator, and the actual `/mcp` URL don't agree
exactly, auth fails with 401.
...
Test it: ask Claude *"remember that we keep birthday cards in the hallway drawer"* (calls
`add_fact`), then in a new message *"where are the birthday cards?"* (calls `search_facts`).
...
> end-to-end. If Part E fails, the WorkOS dashboard logs and the container logs (Scaleway →
> Cockpit/logs) show which leg broke.
